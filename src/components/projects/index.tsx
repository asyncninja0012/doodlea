    ...
    if(!canCreate) {
        return (
            <div className='text-center py-12'>
                <p className='text-lg'>
                    Please signin to view your projects.
                </p>
            </div>
        )
    }
                ...
                <Link key={project.id} href={`/dashboard/${user.slug}/canvas?project=${project._id}`} className='group cursor-pointer'>
                    <div className='space-y-3'>
                        <div className='aspect-[4/3] rounded-lg overflow-hidden bg-muted'>
                            {project.thumbnail ? (
                                <Image src={project.thumbnail} alt={project.name} width={300} height={300} className='w-full h-full object-cover group-hover:scale-105 transition-transform duration-200' />
                            ): (
