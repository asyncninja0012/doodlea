...
const TextSidebar = ({isOpen}: Props) => {
    ...
    return (
        <div className={cn('fixed right-5 top-1/2 transform -translate-y-1/2 w-80 backdrop-blur-xl bg-white/[0.08] border-white/[0.12] gap-2 p-3 saturate-150 border rounded-lg z-50 transition-transform duration-300', isOpen? 'translate-x-0' : 'translate-x-4-full')}>
            <div className="p-4 flex flex-col gap-10 overflow-y-auto max-h-[calc(100vh-8rem)]">
                ...
                <div className='space-y-3'>
                    ...
                    <div className='flex gap-2'>
                        <Toggle pressed={selectedTextShape.fontWeight>=600} onPressedChange = {(pressed)=>updateTextProperty('fontWeight', pressed ? 700 : 400)} className='data-[state=on]:bg:blue-500 data-[state=on]:text-white'>
                            <Bold className="w-4 h-4" />
                        </Toggle>
                        <Toggle pressed={selectedTextShape.fontStyle==='italic'} onPressedChange = {(pressed)=>updateTextProperty('fontStyle', pressed ? "italic" : "normal")} className='data-[state=on]:bg:blue-500 data-[state=on]:text-white'>
                            <Italic className="w-4 h-4" />
                        </Toggle>
                         <Toggle pressed={selectedTextShape.textDecoration==='underline'} onPressedChange = {(pressed)=>updateTextProperty('textDecoration', pressed ? "underline" : "none")} className='data-[state=on]:bg:blue-500 data-[state=on]:text-white'>
                            <Underline className='w-4 h-4' />
                        </Toggle>
                        <Toggle pressed={selectedTextShape.textDecoration==='line-through'} onPressedChange = {(pressed)=>updateTextProperty('textDecoration', pressed ? "line-through" : "none")} className='data-[state=on]:bg:blue-500 data-[state=on]:text-white'>
                ...
                <div className='space-y-2'>
                    <Label className='text-white/80'>Line Height: {selectedTextShape.lineHeight}px</Label>
                    <Slider value={[selectedTextShape.lineHeight]} onValueChange={([value])=>updateTextProperty('lineHeight', value)}
                    min={0.8}
                    max={3}
                    step={0.1}
                    className='w-full' />
                ...
                <div className='space-y-2'>
                    ...
                    <Slider value={[selectedTextShape.letterSpacing]} onValueChange={([value])=>updateTextProperty('letterSpacing', value)}
                    min={-2}
                    max={10}
                    step={0.1}
                    className='w-full' />
                ...
                <div className='space-y-2'>
                    <Label className='text-white/80'><Palette className='w-4 h-4' />Text Color</Label>
                    <div className='flex gap-2'>
                        <Input
                            value={colorInput}
