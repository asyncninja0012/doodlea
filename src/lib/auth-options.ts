...
const VALIDATION_INTERVAL = 0 // Always re-validate — catches deleted users immediately

/**
 * NextAuth configuration — kept in a lib file so it can be imported by
 * server-side code (getServerSession, API routes) without violating Next.js's
 * constraint that route files may only export HTTP method handlers.
 */
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger }) {
      ...
      // Periodic validation — re-check subscription status every 5 min
